 ...
 */
public final class DictionaryVectorizer extends AbstractJob implements
		Vectorizer {
	private static final Logger log = LoggerFactory
			.getLogger(DictionaryVectorizer.class);
	...
	public void createVectors(Path input, Path output, VectorizerConfig config)
		...
		createTermFrequencyVectors(input, output, config.getTfDirName(),
				config.getConf(), config.getMinSupport(),
				config.getMaxNGramSize(), config.getMinLLRValue(),
				config.getNormPower(), config.isLogNormalize(),
				config.getNumReducers(), config.getChunkSizeInMegabytes(),
				config.isSequentialAccess(), config.isNamedVectors());
	}
	 ...
	 *            {@link org.apache.mahout.math.RandomAccessSparseVector}'s of
	 *            the document are generated
	 * @param tfVectorsFolderName
	 *            The name of the folder in which the final output vectors will
	 *            be stored
	 * @param baseConf
	 *            job configuration
	 * @param normPower
	 ...
	 *            increased swapping
	 */
	public static void createTermFrequencyVectors(Path input, Path output,
			String tfVectorsFolderName, Configuration baseConf, int minSupport,
			int maxNGramSize, float minLLRValue, float normPower,
			boolean logNormalize, int numReducers, int chunkSizeInMegabytes,
			boolean sequentialAccess, boolean namedVectors) throws IOException,
		...
		log.info("Creating dictionary from {} and saving at {}", input,
				dictionaryJobPath);
		...
		if (maxNGramSize == 1) {
			...
			dictionaryChunks = createDictionaryChunks(dictionaryJobPath,
					output, baseConf, chunkSizeInMegabytes, maxTermDimension);
		} else {
			CollocDriver.generateAllGrams(input, dictionaryJobPath, baseConf,
					maxNGramSize, minSupport, minLLRValue, numReducers);
			dictionaryChunks = createDictionaryChunks(
					new Path(new Path(output, DICTIONARY_JOB_FOLDER),
		...
		for (Path dictionaryChunk : dictionaryChunks) {
			Path partialVectorOutputPath = new Path(output,
					VECTOR_OUTPUT_FOLDER + partialVectorIndex++);
			partialVectorPaths.add(partialVectorOutputPath);
			makePartialVectors(input, baseConf, maxNGramSize, dictionaryChunk,
					partialVectorOutputPath, maxTermDimension[0],
		...
		PartialVectorMerger.mergePartialVectors(partialVectorPaths, outputDir,
				conf, normPower, logNormalize, maxTermDimension[0],
				sequentialAccess, namedVectors, numReducers);
		HadoopUtil.delete(conf, partialVectorPaths);
	}

	public static void createUpdateTermFrequencyVectors(Path input, Path output,
			String tfVectorsFolderName, Configuration baseConf, int maxNGramSize, 
			String dictionaryPaths, float normPower, boolean logNormalize, int numReducers,
			boolean sequentialAccess, boolean namedVectors) 
	...
	/**
	 * Read the feature frequency List which is built at the end of the Word
	 * Count Job and assign ids to them. This will use constant memory and will
	 * run at the speed of your disk read
	 */
	...
	/**
	 * Create a partial vector using a chunk of features from the input
	 * documents. The input documents has to be in the {@link SequenceFile}
	 * format
	 * 
	 * @param input
	 *            input directory of the documents in {@link SequenceFile}
	 ...
	 * @param namedVectors
	 *            output vectors should be named, retaining key (doc id) as a
	 *            label
	 * @param numReducers
	 ...
	 */
	private static void startWordCounting(Path input, Path output,
			Configuration baseConf, int minSupport) throws IOException,
			InterruptedException, ClassNotFoundException {
		...
		job.setJobName("DictionaryVectorizer::WordCount: input-folder: "
				+ input);
		job.setJarByClass(DictionaryVectorizer.class);
	...
	public int run(String[] args) throws Exception {
		addInputOption();
		addOutputOption();
		addOption("tfDirName", "tf", "The folder to store the TF calculations",
				"tfDirName");
		addOption("minSupport", "s",
				"(Optional) Minimum Support. Default Value: 2", "2");
		...
		addOption(
				"norm",
				"n",
				"The norm to use, expressed as either a float or \"INF\" "
						+ "if you want to use the Infinite norm.  "
						+ "Must be greater or equal to 0.  The default is not to normalize");
		addOption("logNormalize", "lnorm",
				"(Optional) Whether output vectors should be logNormalize. "
						+ "If set true else false", "false");
		addOption(DefaultOptionCreator.numReducersOption().create());
		addOption("chunkSize", "chunk",
				"The chunkSize in MegaBytes. 100-10000 MB", "100");
		addOption(DefaultOptionCreator.methodOption().create());
		addOption("namedVector", "nv",
				"(Optional) Whether output vectors should be NamedVectors. "
						+ "If set true else false", "false");
		...
		createTermFrequencyVectors(getInputPath(), getOutputPath(), tfDirName,
				getConf(), minSupport, maxNGramSize, minLLRValue, normPower,
				logNormalize, numReducers, chunkSizeInMegs, sequential,
				namedVecs);
		return 0;
	}
